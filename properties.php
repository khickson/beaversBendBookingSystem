...
<body>
    ...
    <script>
        function confirmDel(cabinName, pid) {
            url = "property_delete.php?pid=" + pid;
            var agree = confirm("Delete This Cabin: " + cabinName + " ? ");
            if (agree){
                location.href = url;
            }else{
                return;
...
<?php
    ...
    if ($stmt->prepare($sql)) {
        ...
        $tblRows = "";
        while ($stmt->fetch()) {
            $cabinName_js = $cabinName;
            $tblRows = $tblRows . "<tr><td><a href='http://ctec4321.khh9106.uta.cloud/termProject/moreDetails.php?PID=$pid'>$cabinName</a></td><td><a href='propertyForm.php?pid=$pid'><i class=\"fas fa-edit\"></i> Edit</a> | <a href='javascript:confirmDel(\"$cabinName_js\", $pid)'><i class=\"fas fa-trash-alt\"></i> Delete</a></td></tr>\n";
        }

        $output = "
                <table class='cabinList'>
                    <tr><th>Cabin Name</th><th>Options</th></tr>\n" . $tblRows . "
                </table>
        ";
        $stmt->close();
    }
