...
if (isset($_POST['Submit'])){
    $required = array("reserveName", "fromDate", "toDate", "status", "cabinChoice");
    $expected = array("UID", "reserveName", "fromDate", "toDate", "status", "cabinChoice");

    $label= array("UID" => "Reservation ID", "reserveName"=>"Reservation Name", "fromDate"=>"Date From", "toDate"=>"Date To", "status"=>"Status", "cabinChoice"=>"Cabin Choice");
    ...
    if (empty($missing)){
        ...
        if ($uid != ""){
            $uid = intval($uid);
            $sql = "UPDATE reservations SET name = ?, dateFrom = ?, dateTo = ?, status = ?, PID = ?";

            if ($stmt->prepare($sql)){
                $stmt->bind_param('sssii', $reserveName, $fromDate, $toDate, $status, $cabinChoice);
                $stmt_prepared = 1;
            }
        } else{
            $sql = "INSERT INTO reservations (name, dateFrom, dateTo, status, PID) values (?,?,?,?,?)";
            if ($stmt->prepare($sql)){
                $stmt->bind_param('sssii', $reserveName, $fromDate, $toDate, $status, $cabinChoice);
                $stmt_prepared = 1;
            }
    ...
    $output = "<div>Please begin your reservation management operation from the <a href='reservations.php'>Reservation Managment Page</a>.</div>";
}
?>
